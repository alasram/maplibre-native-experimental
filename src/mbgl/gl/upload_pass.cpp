...
void UploadPass::updateTextureResourceSub(gfx::TextureResource& resource,
    ...
    assert(commandEncoder.context.getTexturePool().isUsed(static_cast<gl::TextureResource&>(resource).texture));
    assert(
        commandEncoder.context.getTexturePool().desc(static_cast<gl::TextureResource&>(resource).texture).channelType ==
        type);
    assert(
        commandEncoder.context.getTexturePool().desc(static_cast<gl::TextureResource&>(resource).texture).pixelFormat ==
        format);
    assert(
        commandEncoder.context.getTexturePool().desc(static_cast<gl::TextureResource&>(resource).texture).size.width >=
        xOffset + size.width);
    assert(
        commandEncoder.context.getTexturePool().desc(static_cast<gl::TextureResource&>(resource).texture).size.height >=
        yOffset + size.height);
